...
public class DefaultTaskService implements TaskService {
  ...
  public void updateTask(String taskId, TaskRequest taskRequest) {
    log.debug("updateTask(taskId={}, task={})", taskId, taskRequest);

    Task originalTask = getTaskOrThrowException(taskId); // Make sure this task actually exists
    Task updatedTask = mapperFacade.map(taskRequest, Task.class);

    // Copy immutable properties from the original task
    updatedTask.setId(originalTask.getId());
    updatedTask.setCreated(originalTask.getCreated());
