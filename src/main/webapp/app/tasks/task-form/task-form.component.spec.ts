...
describe('TaskFormComponent', () => {
  ...
  it('should create a task', () => {
    // given
    component.taskForm.setValue({ name: 'My task' });
    taskService.create.and.returnValue(of({ id: 'id', name: 'My task', status: 'pending', done: false }));

    // when
    component.onSubmit();

    // then
    expect(taskService.create).toHaveBeenCalledWith('My task', 'pending');
  });
  ...
  it('should emit the task after creation', () => {
    ...
    // then
    expect(createEmitter).toHaveBeenCalledWith({ id: 'id', name: 'My task', status: 'pending', done: false });
  });
